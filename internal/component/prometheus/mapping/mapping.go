...
type Arguments struct {
	...
	SrcLabelName    string `alloy:"src_label_name,attr"`
	OutputLabelName string `alloy:"output_label_name,attr"`
...
func New(o component.Options, args Arguments) (*Component, error) {
	...
	c := &Component{
		...
		srcLabelName:       args.SrcLabelName,
		outputLabelName:    args.OutputLabelName,
...
func (c *Component) Update(args component.Arguments) error {
	...
	c.srcLabelName = newArgs.SrcLabelName
	c.outputLabelName = newArgs.OutputLabelName
...
func (c *Component) mapping(val float64, lbls labels.Labels) labels.Labels {
	...
	srcValue := lb.Get(c.srcLabelName)
	outputValue := c.mappings[srcValue]
	lb.Set(c.outputLabelName, outputValue)
	newLabels := lb.Labels()
